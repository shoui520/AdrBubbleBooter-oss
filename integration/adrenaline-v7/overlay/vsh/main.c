...
static int sceLsdbGetTypePatched(void *a1) {
  ...
  if (val == 1) { // type normal
	...
	strcpy(app_titleid, name);
	char boot_inf[64], boot_bin[64];
	snprintf(boot_inf, 64, "ux0:app/%s/data/boot.inf", app_titleid);
	snprintf(boot_bin, 64, "ux0:app/%s/data/boot.bin", app_titleid);
	int read = ReadFile(boot_bin, &boot_info, sizeof(BootInfo));
    if ((name && strncmp(name, ADRENALINE_TITLEID, 6) == 0) || ((read >= 0) && (boot_info.magic == 0x00424241)) || (CheckFile(boot_inf) >= 0)) {
...
int module_start(SceSize args, void *argp) {
  ...
  // Destroy
  sceAppMgrDestroyAppByName(app_titleid);
