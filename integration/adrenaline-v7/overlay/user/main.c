...
char app_titleid[12], boot_logo_path[64];
int adr_id = -1, boot_logo = -1;
int ms_location = 0;
...
int SaveState(SceAdrenaline *adrenaline, void *savestate_data) {
  ...
  SceUID fd = sceIoOpen(tmp, SCE_O_WRONLY, 0777);
  if (fd < 0)
    return fd;
...
int AdrenalineCompat(SceSize args, void *argp) {
  ...
  while (1) {
	...
	if((boot_info.loadstate >= 1) && (!load_state))
		autoLoadState(boot_info.loadstate-1);
...
static int sceCompatWaitSpecialRequestPatched(int mode) {
  ...
  if (!adr_id && (boot_info.loadstate == 0 || (boot_info.loadstate > 0 &&
      stateExists(boot_info.loadstate-1) < 0) || (pad.buttons & SCE_CTRL_LTRIGGER))) {
	((uint32_t *)n)[0] = 5;
	load_state = 1;
  }

  if ((pad.buttons & SCE_CTRL_RTRIGGER) && adr_id)
  ...
  if (adr_id) {
     if (config.skip_logo) {
	  ...
	  if (boot_logo) {
		 ...
		 if (sceIoGetstat(boot_logo_path, &stat) >= 0){
			 *png_size = stat.st_size;
			 ReadFile(boot_logo_path, png_buf, stat.st_size);
			 num_startup_png = 1;
	     }
     }
  }
...
int module_start(SceSize args, void *argp) {
  ...
  adr_id = (strcmp(app_titleid, ADRENALINE_TITLEID) == 0) ? 1 : 0;
  ...
  if(adr_id){
	   ...
	   ReadFile(config_path, &boot_info, sizeof(BootInfo));
	   sprintf(config_path, (boot_info.customized) ? "ux0:app/%s/data/config.bin" : "ux0:app/" ADRENALINE_TITLEID "/adrenaline.bin", app_titleid);
       ReadFile(config_path, &config, sizeof(AdrenalineConfig));
	   sprintf(boot_logo_path, "ux0:app/%s/data/boot.png", app_titleid);	
       boot_logo = (CheckFile(boot_logo_path)>=0);
  }

  ms_location = config.ms_location;

  if (config.psp_screen_scale_x <= 0.2f)
    config.psp_screen_scale_x = 2.0f;
  if (config.psp_screen_scale_y <= 0.2f)
    config.psp_screen_scale_y = 2.0f;
  if (config.ps1_screen_scale_x <= 0.2f)
    config.ps1_screen_scale_x = 1.0f;
  if (config.ps1_screen_scale_y <= 0.2f)
    config.ps1_screen_scale_y = 1.0f;

  memset(&menu_color, 0, sizeof(MenuColor)); 
  ReadFile("ux0:app/" ADRENALINE_TITLEID "/menucolor.bin", &menu_color, sizeof(MenuColor));
  initMenuColors();
