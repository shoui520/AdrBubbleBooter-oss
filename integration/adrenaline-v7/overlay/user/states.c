...
void drawStates() {
  ...
  for (i = 0; i < 4; i++) {
    if (i == rel_pos)
      vita2d_draw_rectangle(WINDOW_X, FONT_Y_LINE(2 + i * 5) + 3.0f - FONT_Y_SPACE/2.0f, WINDOW_WIDTH, FONT_Y_SPACE * 5, menu_color.select); //COLOR_ALPHA(RED, 0x8F) //COLOR_ALPHA(0xFFFF1F7F, 0x8F)

    if (states[base_pos + i].num == -1) {
      vita2d_draw_rectangle(75.0f - 55.0f, FONT_Y_LINE(2 + i * 5) + 3.0f, 240.0f * k, 92.0f, menu_color.header); //COLOR_ALPHA(GRAY, 0x7F)
      pgf_draw_textf(250.0f - 55.0f, FONT_Y_LINE(2 + i * 5), menu_color.label, FONT_SIZE, "New State %d", base_pos + i);

    } else {
      // Screenshot
      if (states[base_pos + i].tex)
        vita2d_draw_texture_scale(states[base_pos + i].tex, 75.0f - 55.0f, FONT_Y_LINE(2 + i * 5) + 3.0f, k, k);

      // Title
      pgf_draw_text(250.0f - 55.0f, FONT_Y_LINE(2 + i * 5), menu_color.label, FONT_SIZE, states[base_pos + i].title);
      ...
      pgf_draw_textf(250.0f - 55.0f, FONT_Y_LINE(3 + i * 5), menu_color.label, FONT_SIZE, "%s %s", date_string, time_string);

      // Size
      char string[16];
      getSizeString(string, states[base_pos + i].size);
      pgf_draw_text(250.0f - 55.0f, FONT_Y_LINE(4 + i * 5), menu_color.label, FONT_SIZE, string);	
	  
	  // State number
	  pgf_draw_textf(250.0f - 55.0f, FONT_Y_LINE(5 + i * 5), menu_color.label, FONT_SIZE, "State %d", states[base_pos + i].num);
    }
  }
  ...
  if (open_options) {
    ...
    for (i = 0; i < n_options; i++) {
      uint32_t color = (i == option_sel) ? menu_color.option : menu_color.label;
	  ...
	  pgf_draw_text(ALIGN_RIGHT(SCREEN_WIDTH - 20.0f, vita2d_pgf_text_width(font, FONT_SIZE, option_entries[i])), FONT_Y_LINE(2 + rel_pos * 5 + i), color, FONT_SIZE, option_entries[i]);
    }
  }
}
...
void autoLoadState(int num) {
	if(load_state)
		return;
	if(stateExists(num) < 0)
		return;
	load_state = 1;
	loadState(num);
	base_pos = num;
