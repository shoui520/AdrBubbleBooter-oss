...
static int ksceKernelStartPreloadedModulesPatched(SceUID pid) {
  ...
  if (strcmp(titleid, "main") == 0) {
	...
	char boot_inf[64], boot_bin[64];
	snprintf(boot_inf, 64, "ux0:app/%s/data/boot.inf", titleid);
	snprintf(boot_bin, 64, "ux0:app/%s/data/boot.bin", titleid);
	if (CheckFile(boot_inf) >= 0)
		ksceKernelLoadStartModuleForPid(pid, "ux0:app/" ADRENALINE_TITLEID "/sce_module/bootconv.suprx", 0, NULL, 0, NULL, NULL);
	int read = ReadFile(boot_bin, &boot_info, sizeof(BootInfo));
	if ((strncmp(titleid, ADRENALINE_TITLEID, 6) == 0) || ((read >= 0) && (boot_info.magic == 0x00424241))) {
		ksceKernelLoadStartModuleForPid(pid, "ux0:app/" ADRENALINE_TITLEID "/sce_module/adrbubblebooter.suprx", 0, NULL, 0, NULL, NULL);
