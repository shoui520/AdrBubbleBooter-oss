...
int cpu_list[] = {0, 20, 50, 75, 100, 111, 122, 133, 166, 200, 222, 266, 288, 300, 333};
int bus_list[] = {0, 10, 25, 37, 50, 55, 61, 66, 83, 100, 111, 133, 144, 150, 166};
...
void OnSystemStatusIdle() {
	...
	if (sceKernelBootFrom() == PSP_BOOT_DISC) {
		if (adrenaline->adr_id)
			SetSpeed(cpu_list[config.umdisocpuspeed % N_CPU], bus_list[config.umdisocpuspeed % N_CPU]);
		else
			if (config_booter.cpuspeed > 0)
				SetSpeed(cpu_list[config_booter.cpuspeed % N_CPU], bus_list[config_booter.cpuspeed % N_CPU]);
			else
				SetSpeed(cpu_list[config.umdisocpuspeed % N_CPU], bus_list[config.umdisocpuspeed % N_CPU]);
	}
...
int OnModuleStart(SceModule2 *mod) {
	...
	if (strcmp(modname, "sceLowIO_Driver") == 0) {
        ...
        sctrlSEGetBooterConfig(&config_booter);
		...
		if (sceKernelInitKeyConfig() == PSP_INIT_KEYCONFIG_GAME) {
			if (config_booter.highmemory != 2) {
				if (config_booter.highmemory == 1 || config.forcehighmemory) {
					sctrlHENSetMemory(52, 0);
					ApplyMemory();
				}
			}
