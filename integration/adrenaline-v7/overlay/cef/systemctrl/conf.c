...
int sctrlSEGetBooterConfigEx(BootInfo *config_booter, int size) {
	int k1 = pspSdkSetK1(0);
	
	memset(config_booter, 0, size);
	SceUID fd = sceIoOpen("ms0:/__APPID__/data/boot.bin", PSP_O_RDONLY, 0); // ms0:/__APPID__/data/boot.bin // flash1:/config.booter
	if (fd < 0) {
		pspSdkSetK1(k1);
		return -1;
	}

	int read = sceIoRead(fd, config_booter, size);
...
int sctrlSEGetBooterConfig(BootInfo *config_booter) {
	return sctrlSEGetBooterConfigEx(config_booter, sizeof(BootInfo));
}
