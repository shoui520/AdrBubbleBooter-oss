...
void initAdrenalineInfo() {
	...
	adrenaline->game_mode = sceKernelInitKeyConfig() == PSP_INIT_KEYCONFIG_GAME;
}
...
int adrenaline_thread(SceSize args, void *argp) {
	while (1) {
		...
		switch (adrenaline->psp_cmd) {
			...
			case ADRENALINE_PSP_CMD_RESTART_BOOTER:
			    adrenaline->psp_cmd = ADRENALINE_PSP_CMD_NONE;
				ResumeThreads();
				kuKernelLoadStartModule("flash0:/kd/booter.prx", 0, NULL);
				break;
				
			case ADRENALINE_PSP_CMD_EXIT_VSH:
			    adrenaline->psp_cmd = ADRENALINE_PSP_CMD_NONE;
				ResumeThreads();
				sceKernelExitVSHVSH661(NULL);
				break;
				
			case ADRENALINE_PSP_CMD_SUSPEND:
			    adrenaline->psp_cmd = ADRENALINE_PSP_CMD_NONE;
				SuspendThreads();
				break;
				
			case ADRENALINE_PSP_CMD_RESUME:
			    adrenaline->psp_cmd = ADRENALINE_PSP_CMD_NONE;
				ResumeThreads();
				break;
		}
