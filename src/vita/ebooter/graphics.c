...
void psvDebugScreenInit() {
	g_log_mutex = sceKernelCreateMutex("log_mutex", 0, 0, NULL); /* Serialize text rendering. */

	SceKernelAllocMemBlockOpt opt = { 0 };
	opt.size = sizeof(opt);
	opt.attr = 0x00000004;
	opt.alignment = FRAMEBUFFER_ALIGNMENT;
	SceUID displayblock = sceKernelAllocMemBlock("display", SCE_KERNEL_MEMBLOCK_TYPE_USER_CDRAM_RW, FRAMEBUFFER_SIZE, &opt); /* Allocate CDRAM. */
	printf("displayblock: 0x%08x", displayblock);
	void *base; /* Mapped framebuffer base returned by the kernel. */
	...
	g_vram_base = base; /* Publish the framebuffer to the text renderer. */
	...
	g_fg_color = 0xFFFFFFFF; /* Default text color. */
	g_bg_color = 0x00000000; /* Default background color. */
}
...
static void printTextScreen(const char *text)
{
	...
	for (c = 0; c < strlen(text); c++) {
		...
		vram = getVramDisplayBuffer() + gX + gY * LINE_SIZE; /* Address the glyph's top-left pixel. */
